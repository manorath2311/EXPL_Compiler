...
%token BREAK CONT DECL ENDDECL INT STR MAIN RETURN TYPE
%token<nptr> ENDTYPE NILL DEQNILL NEQNILL FREE ALLOC INIT
...
%type <nptr> program Slist Stmt expr id Type FieldType FType
...
FDef: Type ID '(' ParamList ')' '{' LDeclBlock Body '}' {
                                                            ...
                                                            if(Gtemp == NULL) 
                                                            {
                                                                error("Function %s not declared", $2->name);
                                                                exit(1);
                                                            }

                                                            if(Gtemp->type != declarationType) 
                                                            {
                                                               error("%s : Function type does not match declaration", $2->name);
                                                               exit(1);
                                                            }
                                                            ...
                                                            while(argList1 != NULL && argList2 != NULL) 
                                                            {
                                                                if(argList1->type != argList2->type) {
                                                                    error("%s : Conflict in argument types", $2->name);
                                                                    exit(1);
                                                                }

                                                                if(strcmp(argList1->name, argList2->name)) {
                                                                    error("%s : Conflict in argument names", $2->name);
                                                                    exit(1);
                                                                }
...
AsgStmt: ID ASSGN expr ';'          {
        ...
        /* ID '['NUM']' ASSGN expr ';'    {
                                        $1->type = TLookup("integer");
                                        assignType($1, 2);
                                        typecheck($1->type->ptr1, $6->type, '=');
                                        $$ = TreeCreate(TLookup("void"), NODE_ARRAY_ASSGN, NULL, NULL, NULL, $1, $3, $6);
                                    } */
         /* | ID '[' id ']' ASSGN expr ';'   {
                                         assignType($1, 2);
                                         typecheck($1->type->ptr1, $6->type, '=');
